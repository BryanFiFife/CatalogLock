...
function jsonParse(body, url, findings) {
    try {
    ...
    for (let i = 0; i < candidates.length; i++) {
        ...
        try {
            ...
            if (result.status >= 200 && result.status < 300) {
                ...
                rootFetch = result;
                if (i > 0)
                    findings.push({ ruleId: 'compat/ard-json', severity: 'warning', message: 'Using compatibility /.well-known/ard.json; current ARD publisher path is /.well-known/ai-catalog.json', location: rootUrl });
                break;
            }
            if (result.status !== 404 || i === candidates.length - 1)
                findings.push({ ruleId: 'fetch/http-status', severity: 'error', message: `Catalog returned HTTP ${result.status}`, location: candidate });
        }
        catch (err) {
            findings.push({ ruleId: 'fetch/root', severity: 'critical', message: err instanceof Error ? err.message : String(err), location: candidate });
            if (i === 0)
                break; // security failure must not silently fall through to a second path.
        }
    }
    if (!rootUrl || !rootFetch)
        return { root: candidates[0], catalogs, findings, trust };
    const queue = [{ url: rootUrl, depth: 0, prefetched: rootFetch }];
    while (queue.length) {
        ...
        try {
            ...
            findings.push({ ruleId: 'fetch/catalog', severity: 'error', message: err instanceof Error ? err.message : String(err), location: normalized });
            continue;
        }
        ...
        for (const entry of catalog.entries) {
            ...
            if (entry.type === 'application/ai-catalog+json' && typeof entry.url === 'string') {
                if (item.depth >= policy.maxDepth)
                    findings.push({ ruleId: 'limits/depth', severity: 'error', message: `Nested catalog not followed because depth would exceed ${policy.maxDepth}`, location: entry.url });
                else
