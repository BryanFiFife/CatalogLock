...
function setFailOn(policy, value) {
    ...
    return { ...policy, failOn: value };
}
function shouldFail(findings, threshold) { return findings.some((f) => severityRank(f.severity) >= severityRank(threshold)); }
async function writeOrStdout(content, file) { if (typeof file === 'string') {
...
export async function main(argv = process.argv.slice(2)) {
    ...
    if (command === 'diff') {
        ...
        if (args.format === 'json')
            ...
            const lines = [`CatalogLock diff: ${diff.changed ? 'CHANGED' : 'clean'}`, `Blast radius: ${JSON.stringify(diff.blastRadius)}`, ...diff.changes.map((c) => `[${c.severity.toUpperCase()}] ${c.kind}: ${c.message}`)];
