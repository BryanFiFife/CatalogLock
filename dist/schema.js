...
export function validateCatalog(value, location, policy) {
    ...
    value.entries.forEach((raw, i) => {
        ...
        if (typeof e.displayName !== 'string' || !e.displayName)
            findings.push({ ruleId: 'schema/display-name', severity: 'error', message: 'displayName is required', location: loc });
        if (typeof e.type !== 'string' || !e.type)
            findings.push({ ruleId: 'schema/type', severity: 'error', message: 'type is required', location: loc });
        ...
        if (hasUrl) {
            try {
                ...
                if (policy.requireHttpsEntries && u.protocol !== 'https:')
                    findings.push({ ruleId: 'transport/entry-https', severity: 'error', message: 'Entry URL must use HTTPS', location: loc });
                if (u.username || u.password)
                    findings.push({ ruleId: 'transport/url-credentials', severity: 'critical', message: 'Entry URL must not embed credentials', location: loc });
            }
            catch {
