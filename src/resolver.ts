...
import { assessEntryTrust, type TrustFrameworkVerifier } from './trust.js';
...
function attrs(tag: string): Record<string, string> {
  ...
  const re = /([:\w-]+)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/g;
  for (let m; (m = re.exec(tag));) out[m[1]!.toLowerCase()] = m[2] ?? m[3] ?? m[4] ?? '';
  return out;
}
export function findArdLink(html: string, base: string): string | undefined {
  for (const tag of html.match(/<link\b[^>]*>/gi) ?? []) {
    const a = attrs(tag);
    const rel = (a.rel ?? '').toLowerCase().split(/\s+/);
    if (!rel.includes('ard') || !a.href) continue;
    try { return new URL(a.href, base).toString(); } catch { continue; }
  }
  return undefined;
}
function parseJson(body: string, url: string, findings: Finding[]): unknown {
  try { return JSON.parse(body); } catch { findings.push({ ruleId: 'parse/json', severity: 'critical', message: 'Response is not valid JSON', location: url }); return undefined; }
}

async function resolveRoot(target: string, policy: Policy, fetcher: Fetcher, findings: Finding[]): Promise<{ url?: string; fetch?: FetchResult; kind: ResolveResult['rootSourceKind'] }> {
  ...
  if (explicitPath) {
    try {
      ...
      if (r.status >= 200 && r.status < 300) {
        if (isHtml(r)) {
          ...
          if (!link) { findings.push({ ruleId: 'discovery/rel-ard', severity: 'error', message: 'HTML target does not advertise rel="ard"', location: r.url }); return { kind: 'explicit' }; }
          const mf = await fetcher(link, policy);
          if (mf.status >= 200 && mf.status < 300) return { url: mf.url || link, fetch: mf, kind: 'rel-ard' };
    ...
    } catch (e) { findings.push({ ruleId: 'fetch/root', severity: 'critical', message: e instanceof Error ? e.message : String(e), location: explicitUrl.toString() }); }
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    try {
      ...
      if (r.status >= 200 && r.status < 300) {
        if (i > 0) findings.push({ ruleId: 'compat/ai-catalog-json', severity: 'warning', message: 'Using predecessor /.well-known/ai-catalog.json; ARD v0.91 consumers are required to fetch /.well-known/ard.json', location: r.url || candidate });
        return { url: r.url || candidate, fetch: r, kind: i === 0 ? 'ard' : 'legacy-ai-catalog' };
      }
      if (r.status !== 404 || i === candidates.length - 1) findings.push({ ruleId: 'fetch/http-status', severity: 'warning', message: `Manifest candidate returned HTTP ${r.status}`, location: candidate });
    } catch (e) { findings.push({ ruleId: 'fetch/root-candidate', severity: i === 0 ? 'warning' : 'error', message: e instanceof Error ? e.message : String(e), location: candidate }); }
  }
...
export async function resolveCatalogs(target: string, options: ResolveOptions = {}): Promise<ResolveResult> {
  ...
  const trust: ResolveResult['trust'] = [];
  ...
  if (!root.url || !root.fetch) return { root: fallbackRoot, rootSourceKind: root.kind, ardContextSha256: ARD_CONTEXT_SHA256, catalogs, findings, trust };

  const queue: Array<{ url: string; depth: number; prefetched?: FetchResult }> = [{ url: root.url, depth: 0, prefetched: root.fetch }];
  while (queue.length) {
    const item = queue.shift()!;
    const normalized = new URL(item.url).toString();
    if (seen.has(normalized)) continue;
    if (catalogs.length >= policy.maxCatalogs) { findings.push({ ruleId: 'limits/catalog-count', severity: 'critical', message: `Entry-source graph exceeds maxCatalogs=${policy.maxCatalogs}`, location: normalized }); break; }
    if (item.depth > policy.maxDepth) { findings.push({ ruleId: 'limits/depth', severity: 'error', message: `Nested entry-source depth exceeds maxDepth=${policy.maxDepth}`, location: normalized }); continue; }
    seen.add(normalized);

    let fetched: FetchResult;
    try { fetched = item.prefetched ?? await fetcher(normalized, policy); }
    catch (e) { findings.push({ ruleId: 'fetch/catalog', severity: 'error', message: e instanceof Error ? e.message : String(e), location: normalized }); continue; }
    if (fetched.status < 200 || fetched.status >= 300) { findings.push({ ruleId: 'fetch/http-status', severity: 'error', message: `Nested entry source returned HTTP ${fetched.status}`, location: normalized }); continue; }
    if (isHtml(fetched)) { findings.push({ ruleId: 'parse/unexpected-html', severity: 'error', message: 'Nested entry source returned HTML instead of an ARD manifest', location: normalized }); continue; }
    ...
    for (const entry of catalog.entries) {
      ...
      if (priorSource && priorSource !== normalized) findings.push({ ruleId: 'authority/identifier-collision', severity: 'critical', message: `Identifier ${entry.identifier} is asserted by multiple entry sources`, location: normalized, evidence: { firstSource: priorSource, secondSource: normalized } });
      ...
      if (entry.type === 'application/ai-catalog+json' && typeof entry.url === 'string') {
        if (item.depth >= policy.maxDepth) findings.push({ ruleId: 'limits/depth', severity: 'error', message: `Nested entry source not followed because depth would exceed ${policy.maxDepth}`, location: entry.url });
