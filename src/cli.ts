...
export async function main(argv=process.argv.slice(2)): Promise<number> {
  ...
  if (command==='scan' || command==='lock' || command==='verify') {
    ...
    if (!target) throw new Error(`${command} requires a target`);
    ...
    const old=JSON.parse(await fs.readFile(lockPath,'utf8')) as CatalogLockfile;
  ...
  if (command==='diff') {
    ...
    if(!a||!b) throw new Error('diff requires two lockfiles');
    const old=JSON.parse(await fs.readFile(a,'utf8')) as CatalogLockfile;
    const cur=JSON.parse(await fs.readFile(b,'utf8')) as CatalogLockfile;
