...
test('locks a public MCP tools/list surface', async () => {
    ...
    const requester = async (_url, body, _p, headers) => {
...
test('supports paginated tools/list and deterministic sorting', async () => {
    const policy = mergePolicy();
    let calls = 0;
    const requester = async (_url, body) => {
        calls++;
        if (!body.params.cursor)
            return { url: endpoint, status: 200, body: JSON.stringify({ jsonrpc: '2.0', id: 1, result: { tools: [{ name: 'z_tool', inputSchema: { type: 'object' } }], nextCursor: 'page2' } }) };
...
test('new MCP tool is critical drift even when identity and endpoint are unchanged', async () => {
    const policy = mergePolicy();
    const requesterA = async () => ({ url: endpoint, status: 200, body: JSON.stringify({ jsonrpc: '2.0', id: 1, result: { tools: [{ name: 'get_invoice', inputSchema: { type: 'object' } }] } }) });
    const requesterB = async () => ({ url: endpoint, status: 200, body: JSON.stringify({ jsonrpc: '2.0', id: 1, result: { tools: [
...
test('tool schema mutation is critical drift', async () => {
    const policy = mergePolicy();
    const requester = async () => ({ url: endpoint, status: 200, body: JSON.stringify({ jsonrpc: '2.0', id: 1, result: { tools: [{ name: 'transfer', inputSchema: { type: 'object' } }] } }) });
...
test('authenticated server card is warning by default and error when required', async () => {
    ...
    const authFetcher = async () => ({ url: cardUrl, status: 200, body: JSON.stringify(authCard) });
...
test('end-to-end audit follows catalog -> server card -> tools/list -> lockfile', async () => {
    ...
    const combinedFetcher = async (url) => {
        if (url === catalogUrl)
            return { url, status: 200, body: rootBody };
        if (url === cardUrl)
            return { url, status: 200, body: JSON.stringify(card) };
        return { url, status: 404, body: '' };
    };
    const requester = async () => ({ url: endpoint, status: 200, body: JSON.stringify({ jsonrpc: '2.0', id: 1, result: { tools: [{ name: 'get_invoice', inputSchema: { type: 'object' } }] } }) });
...
test('parses SSE data responses from tools/list', async () => {
    const policy = mergePolicy();
    const requester = async () => ({ url: endpoint, status: 200, contentType: 'text/event-stream', body: 'event: message\ndata: {"jsonrpc":"2.0","id":1,"result":{"tools":[{"name":"sse_tool","inputSchema":{"type":"object"}}]}}\n\n' });
    const r = await inspectMcpSurfaces(resolved, policy, { fetcher, requester });
